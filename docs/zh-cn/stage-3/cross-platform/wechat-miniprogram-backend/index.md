...
先说结论：**小程序需要后端，不等于必须购买 CloudBase。** CloudBase 只是这一篇选择的入门路线，因为它和微信身份、云函数、数据库的衔接最短。公司已经有后端时可以直接接入原来的服务；以后想换成其他云服务，也可以通过 HTTPS 接口连接。
...
这里先不要去购买 19.9 元/月的个人版。如果页面让你选择上海或新加坡、PostgreSQL 和付费套餐，说明你进入了 CloudBase 控制台的新购流程，不是这一篇要走的入口。直接退出购买页，不要点击“立即购买”。
